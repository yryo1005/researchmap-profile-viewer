...
2. ファイル中の `{{PERMALINK}}` を、対象教員のpermalinkにすべて書き換える（3箇所）。
3. `<GitHubユーザー名>` と `<リポジトリ名>` の部分を、実際に公開したGitHub Pagesの値に書き換える。
4. 完成したHTMLを、CMSの「埋め込みHTML」欄にそのまま貼り付ける。

これだけで、このリポジトリに一切手を加えることなく、新しい教員のページを追加できます。

`cms-shell-template.html` 内の `<script>` は、ページの高さをiframeに自動反映させるためのものです。CMS側でHTML欄内の `<script>` 実行が許可されていない場合は、この部分が動作せず、iframeの高さが固定のままになります（本文の表示自体は行われます）。

## 表示される情報

researchmapの公開APIから取得できる範囲で、以下を表示します（該当する情報が登録されていない項目は表示されません）。
